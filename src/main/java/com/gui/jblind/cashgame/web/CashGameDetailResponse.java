...
public record CashGameDetailResponse(String id, String name, LocalDateTime scheduledAt, BigDecimal minBuyIn,
		...
		List<CashGamePlayerResponse> players, List<CashGameLogResponse> logs) {

	public static CashGameDetailResponse of(CashGame entity, List<CashGameLogResponse> logs) {
		return new CashGameDetailResponse(entity.getId(), entity.getName(), entity.getScheduledAt(),
				...
				entity.getStatus().name(), entity.getPlayers().stream().map(CashGamePlayerResponse::of).toList(), logs);
	}
